...
function mountLetsScroll(container, config) {
  ...
  if (config.brand) {
    ...
    if (config.brand.logo) {
      // An image logo replaces the generated mark + name entirely.
      const im = el('img', 'sw-brand__logo');
      im.src = config.brand.logo; im.alt = config.brand.name || '';
  ...
  SECTIONS.forEach((s, i) => {
    ...
    const T = i === 0 ? 'h1' : 'h2';  // the opening scene carries the page's h1
    c.innerHTML =
      `<span class="sw-copy__num">${pad(i + 1)} / ${pad(N)}</span>` +
      (s.eyebrow ? `<span class="sw-copy__eyebrow">${esc(s.eyebrow)}</span>` : '') +
      (s.title ? `<${T} class="sw-copy__title">${esc(s.title)}</${T}>` : '') +
  ...
  function read() {
    ...
    // Outro: content after the track scrolls OVER the film; fade the fixed
    // chrome (topbar, route rail, progress bar, held finale copy) out as it
    // approaches so nothing pokes out from behind the page sections.
    const over = smooth(clamp((y - totalW * vh + 0.25 * vh) / (0.75 * vh)));
    const chrome = String(1 - over);
    topbar.style.opacity = chrome; route.style.opacity = chrome; scrollbar.style.opacity = chrome;
    topbar.style.pointerEvents = route.style.pointerEvents = over > 0.5 ? 'none' : '';
    let ci = 0;
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      let outside = 0;
      // The final segment never fades past its end — it holds as the backdrop
      // until the page content scrolls over it (outro).
    ...
    for (let i = 0; i < N; i++) {
      ...
      else if (i === N - 1) cop = (before ? 0 : smooth(pr / 0.4)) * (1 - over); // holds CTA, then yields to the outro
...
function injectCSS() {
  ...
  .sw-brand__logo{height:52px;width:auto;display:block;}
  @media (max-height:820px) and (min-width:861px){
    .sw-copy__title{font-size:clamp(1.7rem,3.4vw,2.5rem);}
    .sw-copy__body{margin-top:12px;}
    .sw-copy__tags{margin-top:16px;}
    .sw-copy__cta{margin-top:18px;}
  }
  @media (max-width:860px){ .sw-brand__logo{height:42px;} }
  ...
  .sw-route__dot{position:relative;border:0;background:transparent;cursor:pointer;width:24px;height:24px;display:grid;place-items:center;}
  ...
  @media (min-width:1100px) and (hover:hover){
    .sw-route__label{opacity:.7;transform:translateY(-50%) translateX(0);}
